...
try {
   $koneksi = db_connect();
   $sql = "select nik_pemohon from perubahan_kk";
   $result = $koneksi->query($sql);
   if($result->num_rows == 0){
       throw new Exception("Data Kosong, tidak ada data yang bisa dihapus");
   }
   while ($row=$result->fetch_array()){
       $sql2 = "delete from perubahan_kk where nik_pemohon='".$row['nik_pemohon']."'";
       $result2 = $koneksi->query($sql2);
       if(!$result2){
           throw new Exception("gagal menghapus data dengan nik_pemohon = ".$row['nik_pemohon']);
       }
	   # menghapus daftar anggota pemohon kk
	   $sql3 = "select * from daftar_anggota_pemohon_kk_2 where nik_pemohon='".$row['nik_pemohon']."'";
	   $result3 = $koneksi->query($sql3);
	   while($row2 = $result3->fetch_array()){
		   $sql4 = "delete from daftar_anggota_pemohon_kk_2 where nik_anggota='".$row2['nik_anggota']."'";
   ...
   alert_sukses("Berhasil reset database f116");
   display_footer();
