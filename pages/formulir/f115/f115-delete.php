...
try {
    $koneksi = db_connect();
    $sql = "Delete From pemohon_kk where nik_pemohon='".$nik_pemohon."'";
	...
	$sql2 = "select * from daftar_anggota_pemohon_kk where nik_pemohon='".$nik_pemohon."'";
	$result2 = $koneksi->query($sql2);
	if($result2->num_rows == 0){
		throw New Exception("Tidak ada data nik pemohon pada tabel daftar anggota pemohon kk");
	}
	while($row = $result2->fetch_array()){
		$sql3 = "delete from daftar_anggota_pemohon_kk where nik_anggota ='".$row['nik_anggota']."'";
