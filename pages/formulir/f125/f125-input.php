...
switch($step){
	...
	case 3:
		...
		try{
			...
			display_sidebar();
			display_content("Formulir", "?page1=formulir", "F-1.25");
			form_input_f125_step3($jumlah_anggota_keluarga_yang_pindah);
			display_footer();
		}catch(Exception $e){
			display_header("Formulir F-1.25");
			display_sidebar();
	...
	case 4:
		try {
			...
			display_footer();
			header("refresh:2;url=?page1=f125");
		}catch(Exception $e){
			# hapus session 
			foreach($_SESSION as $kunci => $nilai){
				if($kunci != "loggedin"){
            ...
            alert_gagal($e->getMessage());
            display_footer();
			header("refresh:3;url=?page1=f125");
		}
		break;
}
